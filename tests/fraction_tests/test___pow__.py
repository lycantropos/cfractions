...
def test_exponents_add(
    base: Fraction, first_exponent: Fraction, second_exponent: Fraction
) -> None:
    assert base ** (first_exponent + second_exponent) == (
        base**first_exponent
    ) * (base**second_exponent)
...
@given(
    strategies.fractions,
    strategies.rationals,
    strategies.small_non_negative_integral_fractions,
)
def test_mul_operand(
    first_base: Fraction, second_base: Rational, exponent: Rational
) -> None:
    second_base_power = second_base**exponent

    assert (
        not isinstance(second_base_power, numbers.Rational)
        or (first_base * second_base) ** exponent
        == (first_base**exponent) * second_base_power
    )
...
def test_rmul_operand(
    first_base: int, second_base: Fraction, exponent: Rational
) -> None:
    first_base_power = first_base**exponent

    assert not isinstance(first_base_power, numbers.Rational) or (
        (first_base * second_base) ** exponent
        == first_base_power * second_base**exponent
    )
...
def test_rtruediv_operand(
    first_base: int, second_base: Fraction, exponent: Fraction
) -> None:
    assert (first_base / second_base) ** exponent == (first_base**exponent) / (
        second_base**exponent
    )


@given(
    strategies.int64_fractions, strategies.small_non_negative_integral_floats
)
